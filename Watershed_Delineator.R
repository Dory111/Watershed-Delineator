...
rast <- rast('C:/Users/ChristopherDory/LWA Dropbox/Christopher Dory/Projects/598/598.06/00 ISW/Data/Raster/DEM/USGS_13_n42w123_20231102.tif')
ext <- st_read('C:/Users/ChristopherDory/LWA Dropbox/Christopher Dory/Projects/598/598.06/6b&d Upland Management/Data/Shapefiles/Scott_HUC_12.shp')
ext <- st_transform(ext,4269)
ext <- ext[ext$name == 'South Fork Scott River', ]
...
# outlet_location <- st_sf(st_sfc(st_point(x = c(-122.81037403743431, 41.296748172004925)),
#                                 crs = 4326))
# st_geometry(outlet_location) <- 'geometry'
# outlet_location2 <- st_sf(st_sfc(st_point(x = c(-122.81665124234362, 41.29603443197336)),
#                                 crs = 4326))
# st_geometry(outlet_location2) <- 'geometry'
# outlet_location <- rbind(outlet_location, outlet_location2)
# rast <- crop(rast, st_buffer(outlet_location, 100))
...
Watershed_Delineator(raster = rast,
                     ...
                     outlet_location = matrix(c(-122.81605929858031, 41.294905989735675,
                                                -122.81646327611723, 41.29473499371626,
                                                -122.81642344734597, 41.29462527103437,
                                                -122.81586774115668, 41.294652345479534), ncol = 2, byrow = TRUE),
                     outlet_location_CRS = 4326,
                     outlet_location_is_sf = FALSE,
                     outlet_location_is_line = TRUE)
a <- rast('C:/Users/ChristopherDory/LWA Dropbox/Christopher Dory/Projects/598/598.06/00 ISW/Output/Raster/Flow_Dir_Test.tif')
...
plot(a$radians,legend = T,
     main = 'Slope of South Fork')

b <- a
c <- a
values(b)[values(a) != -4444] <- NA
values(b)[values(a) == -4444] <- 1
values(c)[values(a) != -9999] <- NA
values(c)[values(a) == -9999] <- 1

values(a)[values(a) <= -4444] <- NA
plot(a$degrees, add = F, legend = T)
plot(b$degrees, add = T, col = 'pink', legend = F)
plot(c$degrees, add = T, col = 'red', legend = F)
dev.off()
